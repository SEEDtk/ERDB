...
At each point in the result set, the iterator returns a B<ERDB::Object>. The ERDB::Object allows the
...
The L<ERDB::Helpers::SQLBuilder> object describing the query components.
...
Retrieve a record from this query. The record returned will be a B<ERDB::Object>, which
...
sub Fetch {
    ...
    if (@row == 0) {
        ...
        $retVal = ERDB::Object->_new($self->{_helper}, \@row);
        # Count this result.
        $self->{_results}++;
    }
    # Return the result.
...
(which will be a subclass of L<ERDB::Type>. If the field name is invalid, one
...
Field name to examine, in the L<standard field name format|ERDB/Standard Field Name Format>.
...
=head3 _new
    ...
    my $query = ERDB::Query->new($database, $sth, $sqlHelper);
...
L<ERDB? object for the relevant database.
...
L<ERDB::Helpers::SQLBuilder> object used to build this query.
=back
