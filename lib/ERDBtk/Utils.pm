...
=head1 ERDBtk Maintenance Utility Helper

This object provides utility services for the ERDBtk object that are likely to be of use in
creating maintenance scripts. Unlike normal ERDBtk methods, the methods of this object
...
=head3 new
    ...
    my $utils = ERDBtk::Utils->new($erdb);

Create a new, blank ERDBtk Maintenance Utility Helper.

=over 4

=item erdb

An L<ERDBtk> object for accessing the relevant database.
...
=head3 init_options
    ...
    my @opt_specs = ERDBtk::Utils::init_options()
            ...
            for my $index (keys %$realIndexH) {
                ...
                if (! exists $indexH->{$index}) {
                    ...
                    if ($ndx > $realNdx) {
                        ...
                        my @fields = map { $_ =~ /^(\S+)/; ERDBtk::_FixName($1) } @$fieldsL;
                        my $i = 0;
