...
=head1 ERDBtk Query Console Helper

=head2 Introduction

This is a simple helper class used by the ERDBtk Query Console.  To construct
...
L<ERDBtk> database object for the current database.

=item query

L<ERDBtk::Query> object for obtaining the query results.
...
=head3 new
    ...
    my $eq = ERDBtk::QueryConsole->new($db, %options);

Construct a new ERDBtk::QueryConsole object. The parameters are as follows.

=over 4

=item db

Database against which to run the query. This can be either an L<ERDBtk>
...
sub new {
    ...
    my $erdb;
    if (! ref $db) {
        $erdb = ERDBtk::GetDatabase($db);
    } else {
        $erdb = $db;
    }
    # Create the ERDBtk::QueryConsole object.
    my $retVal = {
                    erdb => $erdb,
                    secure => $secure,
                    raw => $raw,
...
the C<$ERDBtkExtras::query_limit> parameter. If the user is privileged,
...
sub Submit {
    ...
    my $parmCount = ERDBtk::CountParameterMarks($filterString);
        ...
        if (@nameErrors) {
            ...
            if (! $self->{secure}) {
                ...
                if ($filterString =~ /(.*)LIMIT\s+(\d+)(.*)/) {
                    ...
                    if ($2 >= $ERDBtkExtras::query_limit) {
                        $filterString = "$1LIMIT $ERDBtkExtras::query_limit$3";
                    }
                } else {
                    # No limit present, so add one.
                    $limitClause = " LIMIT $ERDBtkExtras::query_limit";
                }
            } else {
...
=head3 SplitFields
    ...
    my @fields = ERDBtk::QueryConsole::SplitFields($fieldString);
...
=head3 Quotify
    ...
    my $quoted = ERDBtk::QueryConsole::Quotify($string, $q);
...
=head3 DQuotify
    ...
    my $quoted = ERDBtk::QueryConsole::DQuotify($string, $q);
