...
=head1 ERDBtk Pseudo-Documentation Page

=head2 Introduction

This module is used to generate a small pseudo-documentation page for ERDBtk. The
...
L<ERDBtk> object describing the database.
...
Name of the relevant ERDBtk database.

=item dbObject

L<ERDBtk> object for the databasse. If this option is specified, it
overrides =name=.
...
Create a documentation widget for the current ERDBtk database. The documentation
...
the document needs to have C<ERDBtk.js> included for scripting and C<ERDBtk.css>
...
sub DocPage {
    ...
    for my $type (@types) {
        ...
        if (scalar @$thisGroup) {
            push @optGroupList, CGI::optgroup(-name => ERDBtk::Plurals($type),
                                              -values => $thisGroup);
        }
    }
...
L<ERDBtk> metadata object describing the entity, relationship, or shape.
...
L<ERDBtk> metatada structure for the specified entity.
...
sub DocEntity {
    ...
    push @lines, ERDBtk::ObjectNotes($metadata, $self);
...
L<ERDBtk> metadata structure for the relationship.
...
L<ERDBtk> metadata structure for the shape.
...
L<ERDBtk> metadata structure for the specified entity or relationship.
...
sub DocIndexes {
    ...
    for my $relation (sort keys %$relations) {
        ...
        for my $index (sort keys %$indexes) {
            ...
            # Compute its notes.
            my $notes = join("\n", ERDBtk::ObjectNotes($indexData, $self));
            # Compute its type.
...
L<ERDBtk> metadata structure for the specified entity or
relationship.
...
=head3 FancyTable
    ...
    my $html = ERDBtk::PDocPage::FancyTable(\@cols, @rows);
...
sub BuildDiagram {
   ...
   # Compute the URL of the DBD.
    ...
    my $dwriter = qq(<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" ) .
                  ...
                  qq(<param name="movie" value="$ERDBtkExtras::diagramURL" /> <param name="quality" value="high" /> ) .
                  qq(<param name="bgcolor" value="" /> <param name="base" value="$base" /> <param name="swliveconnect" value="" /> ) .
                  qq(<embed src="$ERDBtkExtras::diagramURL" quality="high" bgcolor="" width="$width" ) .
    ...
    push @retVal, ERDBtk::ObjectNotes($erdb->{_metaData}, $self);
    # Return the result.
    return join("\n", @retVal);
}
...
The methods in this section create the appropriate HTML markup for ERDBtk
object notes. It allows this object to be used as a drop-in replacement
for L<WikiTools> when using the L<ERDBtk> documentation methods.
