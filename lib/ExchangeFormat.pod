...
In a typical exchange format directory, the two repositories are directly below the root. The genome
repository is named C<GenomeData> and the subsystem repository C<SubSystemData>. The L<CopyFromSeed.pl>
script creates the directories in this format.
