...
package ERDBtkExtras;

=head1 ERDBtk Extras

=head2 Introduction

This module contains parameter declarations used by the ERDBtk system.
...
=head3 customERDBtktypes

C<$ERDBtkExtras::customERDBtktypes> contains a list of the custom ERDBtk types associated
...
our $customERDBtktypes        = [qw(DNA Link Image LongString SemiBoolean Rectangle CountVector
                               ProteinData ShortString)];

=head3 sort_options

C<$ERDBtkExtras::sort_options> specifies the options to be used when performing a
...
C<$ERDBtkExtras::temp> specifies the name of the directory to be used for
...
C<$ERDBtkExtras::temp_url> must be the URL that can be used to find the temporary
...
C<$ERDBtkExtras::delete_limit> specifies the maximum number of database rows that should
...
C<$ERDBtkExtras::diagramURL> specifies the URL of the ERDBtk diagramming engine.
...
C<$ERDBtkExtras::query_limit> specifies the maximum number of rows that can be
...
C<$ERDBtkExtras::query_retries> specifies the number of times a lost connection
...
C<$ERDBtkExtras::sleep_time> specifies how many seconds to wait between database
